...
function getproducts(){
    ...
    //    condition is check isset or not
          if(!isset($_GET['category'])){
            if(!isset($_GET['brand'])){
     $select_query="Select * from `products` order by rand() limit 0,4"; //limit 0,9 - (to show data in fronyt page)
                    ...
                    $product_price=$row['product_price'];
                     $category_id=$row['category_id'];
                      ...
                      echo " <div class='col-md-4 mb-2'>
                        ...
                        alt='$product_title'><div class='card-body'>
...
function get_all_products(){
    ...
    //    condition is check isset or not
          if(!isset($_GET['category'])){
            if(!isset($_GET['brand'])){
     $select_query="Select * from `products` order by rand()"; //limit 0,9 - (to show data in fronyt page)
            $result_query=mysqli_query($con, $select_query);
            while( $row=mysqli_fetch_assoc( $result_query)){
                $product_id=$row['product_id'];
                 $product_title=$row['product_title'];
...
function get_unique_categories(){
    ...
    //    condition is check isset or not
     ...
     $select_query="Select * from `products` where category_id= $category_id"; //limit 0,9 - (to show data in fronyt page)
            $result_query=mysqli_query($con, $select_query);
...
// getting unique brands
    ...
    //    condition is check isset or not
          if(isset($_GET['brand'])){
            $brand_id=$_GET['brand'];
     $select_query="Select * from `products` where brand_id= $brand_id"; //limit 0,9 - (to show data in fronyt page)
            $result_query=mysqli_query($con, $select_query);
            $num_of_rows=mysqli_num_rows( $result_query);
            if($num_of_rows==0){
...
// displaying brands in sidenav
    ...
    // searching product function
     ...
     $search_query="Select * from `products` where product_keywords like '%$search_value%'"; //limit 0,9 - (to show data in fronyt page)
            $result_query=mysqli_query($con,$search_query);
             $num_of_rows=mysqli_num_rows( $result_query);
            if($num_of_rows==0){
               echo "<h3 class='text-center text-danger'>Sorry! Search Result not found!</h3>";
            }
            while( $row=mysqli_fetch_assoc( $result_query)){
                $product_id=$row['product_id'];
                 $product_title=$row['product_title'];
...
// view details functions
   function view_details(){
    ...
    if(isset($_GET['product_id'])){
     ...
     $select_query="Select * from `products` where product_id='$product_id'"; //limit 0,9 - (to show data in fronyt page)
            $result_query=mysqli_query($con,$select_query);
            while($row=mysqli_fetch_assoc($result_query)){
                $product_id=$row['product_id'];
                 $product_title=$row['product_title'];
